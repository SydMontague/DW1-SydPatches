...
namespace
{
    ...
    struct InfoView
    {
        static constexpr RECT finalPos = {.x = INFO_X, .y = INFO_Y, .width = INFO_WIDTH, .height = INFO_HEIGHT};

        InfoView() = default;
        InfoView(ItemType type, RECT startPos)
            : type(type)
            , box(finalPos, "ITEM DESCRIPTION", LABELED_PANEL_STYLE, TEXT_CYAN, startPos)
        {
        ...
        void tickInput();
        void render();
        bool isOpen() { return box.isOpen(); }
        bool isClosed() { return box.isClosed(); }

    private:
        void updateDescription(ItemType type);
    ...
    void InfoView::updateDescription(ItemType type)
    {
        constexpr int16_t descX = INFO_X + 8;
        constexpr int16_t descY = INFO_Y + 36;
        ...
        itemNameStr = getAtlasVanilla().render(getItem(type)->name,
                                               {.x     = static_cast<int16_t>(box.finalPos().x + 32),
                                                ...
                                                .color = TEXT_CYAN});
        headlineStr = getAtlasVanilla().render(info.headline, {.x = descX, .y = INFO_Y + 86, .color = TEXT_CYAN});
        detailStr   = getAtlasVanilla().render(detail, {.x = descX, .y = INFO_Y + 98, .color = TEXT_LIGHT});
    }
    ...
    void CapacityBar::update()
    {
        ...
        capacityUsedStr =
            getAtlas7px().render(format<8>("%d", countUsedSlots()).data(), {.x = numX, .y = numY, .color = TEXT_CYAN});
        const int16_t usedW  = static_cast<int16_t>(capacityUsedStr.getWidth());
        const int16_t slashX = numX + usedW + 2;
        capacityMaxStr = getAtlas7px().render(format<8>("%d", INVENTORY_SIZE).data(),
    ...
    void ListView::render()
    {
        ...
        for (int32_t p = firstPos; p < lastPos && p < inv_filteredCount; p++) {
            ...
            getAtlasVanilla().renderSlow(format<8>("x%d", INVENTORY_ITEM_AMOUNTS[i]).data(),
                                         depth,
                                         {.x      = posX,
        ...
        if (rowOffset + visibleRows < totalRows) renderScrollArrow(arrowCx, listBottomY - 5 + bop, true, depth);
        ...
        if (totalRows > visibleRows) {
            ...
            const int16_t thumbY =
                static_cast<int16_t>(trackY + ((trackH - thumbH) * rowOffset) / (totalRows - visibleRows));
    ...
    void InfoView::render()
    {
        ...
        renderItemSprite(type, bx + 10, by + 10, depth);
    ...
    void OptionsView::open()
    {
        ...
        menuUseStr           = getAtlasVanilla().render("Use", {.x = labelX, .y = labelY, .color = colorFor(useColor)});
        menuDropStr          = getAtlasVanilla().render(
            "Drop",
            {.x = labelX, .y = static_cast<int16_t>(labelY + 18), .color = colorFor(dropColor)});
        if (allTab)
            menuMoveStr = getAtlasVanilla().render(
                "Move",
                {.x = labelX, .y = static_cast<int16_t>(labelY + 36), .color = colorFor(moveColor)});
        else
            menuMoveStr = {};
    ...
    void ListView::tickInput()
    {
        ...
        if (inv_filteredCount > 0) {
            ...
            int32_t deltaRow = 0;
            int32_t deltaCol = 0;
            if (isKeyRepeat(InputButtons::BUTTON_UP))
                deltaRow = -1;
            else if (isKeyRepeat(InputButtons::BUTTON_DOWN))
                deltaRow = +1;
            if (isKeyRepeat(InputButtons::BUTTON_LEFT))
                deltaCol = -1;
            else if (isKeyRepeat(InputButtons::BUTTON_RIGHT))
                deltaCol = +1;
    ...
    void InventoryUI::tick()
    {
        ...
        if (focusedWindow == FOCUS_INFO) infoView.tickInput();
        ...
        switch (state) {
            case 0:
            {
                ...
                rebuildFiltered();
                INVENTORY_POINTER     = inv_filteredCount > 0 ? inv_filteredIdx[0] : 0;
                capacityBar.capBorder = LabeledUIBox({.x = CAP_X, .y = CAP_Y, .width = CAP_WIDTH, .height = CAP_HEIGHT},
                                                     "CAPACITY",
                                                     LABELED_PANEL_STYLE,
                                                     TEXT_CYAN);
                capacityBar.fullBadgeStr =
                    getAtlas7px().render("FULL",
                                         {.x = CAP_X + CAP_WIDTH - 32 - 6 + 4, .y = CAP_Y + 4 + 1, .color = TEXT_RED});
                listView.bakeAll();
                listView.open();
                capacityBar.update();
            ...
            case 70:
            {
                infoView      = InfoView(INVENTORY_ITEM_TYPES[INVENTORY_POINTER], listView.getCursorRect());
    ...
    void renderInventoryUI(int32_t /*instanceId*/)
    {
        data->render();
    }
