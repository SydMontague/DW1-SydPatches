...
namespace
{
    ...
    // stop the letters from blurring into the background; baseClut = 1 selects
    // the shadow CLUT (semi-trans black at diagonally-offset pixels).
    constexpr RGB8 TAB_ACTIVE_COLOR   = {.red = 0xFF, .green = 0xFF, .blue = 0x64};
    constexpr RGB8 TAB_INACTIVE_COLOR = {.red = 0xFF, .green = 0xFF, .blue = 0xFF};
...
void rebuildFiltered()
{
    ...
    if (!inBattle && inv_categoriesPresent != 0) inv_categoriesPresent |= static_cast<uint8_t>(1 << ALL_CATEGORY);
    ...
    for (int32_t i = 0; i < INVENTORY_SIZE; i++) {
        ...
        if (isAll || buckets[i] == target) inv_filteredIdx[inv_filteredCount++] = static_cast<int8_t>(i);
    }
}
                         ...
                         int32_t depth)
{
    ...
    // (PANEL_FILL_COLOR = 12,22,30, shared by every panel in this menu).
    GsBOXF erase{
        .attribute = 0,
...
void renderCategoryTabs(int16_t tabY, int32_t depth, int16_t* outActiveX, int16_t* outActiveW)
{
    *outActiveX = 0;
    *outActiveW = 0;
    ...
    for (int32_t k = 0; k < 7; k++) {
        ...
        getAtlas7px().renderSlow(CATEGORY_LABELS[i],
                                 depth - 1,
                                 {.x        = labelX,
                                  .y        = static_cast<int16_t>(tabY + 2),
                                  .baseClut = static_cast<uint8_t>(active ? 1 : 0),
